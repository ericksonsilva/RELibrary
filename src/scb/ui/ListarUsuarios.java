...
public class ListarUsuarios extends javax.swing.JInternalFrame {
    ...
    private ListarUsuarios() {
        ...
        tabelaUsuarios.setModel(tableModel);
        pesquisarUsuario();        
    }
    
    public static ListarUsuarios getInstance(){
        if(singleton == null){
            singleton = new ListarUsuarios();
        }
        return singleton;
    }
    
    public static ListarUsuarios newInstance(){
        singleton = new ListarUsuarios();
        return singleton;
    }    
    
    public void atualizarTabela(Usuario u){
        tableModel.setValueAt(u, tabelaUsuarios.getSelectedRow());
    }
 
    public void pesquisarUsuario(){
        try {
            usuarios = facade.getUsuario("%"+c_Pesquisar.getText()+"%");  
            tableModel.addListaDeUsuarios(usuarios);
    ...
    public void removerUsuarioTabela(){
        tableModel.removeUsuario(tabelaUsuarios.getSelectedRow());
    }
    
    public Usuario getSelecionado(){
        return tableModel.getUsuario(tabelaUsuarios.getSelectedRow());
    }
    ...
    private void initComponents() {
        ...
        sp_Cadastros.setViewportView(tabelaUsuarios);

        d_Text1.setText("Pesquise pelo nome ou login:");
    ...
    private void b_VisualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_VisualizarActionPerformed
        ...
        if (tabelaUsuarios.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Escolha o usuário a ser visualizado.", "Erro de seleção", JOptionPane.ERROR_MESSAGE);
        }
        else{
            exibirUsuario = new ExibirUsuario(getSelecionado());

            Main.desktop.remove(exibirUsuario);
            Main.desktop.add(exibirUsuario);
            this.dispose();
            exibirUsuario.show();
        }
    ...
    private void b_ExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_ExcluirActionPerformed
        // TODO add your handling code here:
          if (tabelaUsuarios.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Escolha o usuário a ser excluído.", "Erro de seleção", JOptionPane.ERROR_MESSAGE);
        }
        else{
            int choose = JOptionPane.showConfirmDialog(null, "Você tem certeza que deseja excluir esse usuário?", "Excluir",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
            if(choose == 0){
                try {
                    facade.excluirUsuario(getSelecionado().getId());
                    removerUsuarioTabela();
                } catch (SCBFacadeException ex) {
                    Logger.getLogger(ListarUsuarios.class.getName()).log(Level.SEVERE, null, ex);
                }
          }    
    ...
    private void b_EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_EditarActionPerformed
        // TODO add your handling code here:
        if (tabelaUsuarios.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Escolha o usuário a ser editado.", "Erro de seleção", JOptionPane.ERROR_MESSAGE);
        }
        else{
            editarUsuario = new EditarUsuario(getSelecionado());
            
            Main.desktop.remove(editarUsuario);
            Main.desktop.add(editarUsuario);
            this.dispose();
            editarUsuario.show();                   
...
private ExibirUsuario exibirUsuario;
private UsuarioTableModel tableModel; 
private static ListarUsuarios singleton;
